...
public class AnimationMain {
	...
	private Image crosshair = Toolkit.getDefaultToolkit().getImage(gc.getClass().getClassLoader().getResource("crosshair.png"));
	private Image dartboardImg = Toolkit.getDefaultToolkit().getImage(gc.getClass().getClassLoader().getResource("dartboard.png"));
	private Image reloadButton = Toolkit.getDefaultToolkit().getImage(gc.getClass().getClassLoader().getResource("reload button.png"));
	private Image bulletholeImg = Toolkit.getDefaultToolkit().getImage(gc.getClass().getClassLoader().getResource("bullethole.png"));
	private Image gunshotFire = Toolkit.getDefaultToolkit().getImage(gc.getClass().getClassLoader().getResource("gunshot fire.png"));
		...
		synchronized(gc) {
			...
			if (shotFired || fireCounter < 3) {
				if (pistolX + moveX > GRWIDTH / 3)
					gc.drawImage(gunshotFire, (int)(pistolX - (GRHEIGHT / 5) * 1.85) + moveX, pistolY - GRHEIGHT / 5, 1024, 623);
				else
					gc.drawImage(gunshotFire, (int)(pistolX - (GRHEIGHT / 5) * 0.5) + moveX, pistolY - GRHEIGHT / 5, 1024, 623);
			...
			if (reloading) {
				...
				gc.fillArc(ReloadButton.x - (ReloadButton.width / 6), ReloadButton.y - (ReloadButton.width / 6), ReloadButton.width * 8 / 6, ReloadButton.width * 8 / 6, 0, reload);
			...
			for (int b = 0; b < bulletsLeft; b++) {
				gc.drawImage(bullet, ReloadButton.width * 8 / 6 + (GRWIDTH / 20) + b*(ReloadButton.width / 3 + 1), GRHEIGHT - (GRWIDTH / 10), ReloadButton.width / 3, (int)(ReloadButton.width / 3 * (2.68421)));
			}
